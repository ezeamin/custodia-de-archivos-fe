...
const MenuModuleButton = (props: MenuModuleButtonProps): JSX.Element | null => {
  ...
  return (
    <Link
      className={`bg-sky-100 dark:bg-slate-600 dark:text-white duration-300 flex gap-x-4 hover:bg-sky-200 hover:dark:bg-gray-700 items-center mb-5 px-4 py-2 rounded-lg shadow tooltip tooltip-right h-12 ${
        open ? 'w-full' : 'w-12'
      }`}
      data-tip={!open ? el.description : null}
      to={el.path}
    >
      <Icon className="duration-200" iconComponent={el.icon} title={el.title} />
      <span
